...
const CODEX_COMPAT_FIELDS_TO_REMOVE = [
  'temperature',
  'top_p',
  'max_output_tokens',
  'user',
...
function cleanupCodexRequestBody(body) {
  if (!body || typeof body !== 'object') {
    return
  }

  for (const field of CODEX_COMPAT_FIELDS_TO_REMOVE) {
    delete body[field]
  }
}

function normalizeLegacyTextValue(value, depth = 0) {
  if (depth > 5) {
    return ''
  }
  ...
  if (Array.isArray(value)) {
    return value.map((item) => normalizeLegacyTextValue(item, depth + 1)).join('')
  }
  ...
  if (value && typeof value === 'object') {
    ...
    if (value.text !== undefined) {
      return normalizeLegacyTextValue(value.text, depth + 1)
    }
    return ''
  }
...
function normalizeLegacyCodexInput(body) {
  ...
  for (const item of body.input) {
    ...
    const contentItems = Array.isArray(item.content) ? item.content : [item.content]
    for (const contentItem of contentItems) {
      ...
      if (Array.isArray(contentItem.text)) {
        contentItem.text = normalizeLegacyTextValue(contentItem.text)
      }
    }
  }
}
...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    const codexCliInfo = parseCodexCliUserAgent(userAgent)
    const isCodexCLI = Boolean(codexCliInfo)
    const isLegacyCodexCLI = isCodexCLI && isLegacyCodexCliUserAgent(userAgent, '0.70.0')

    // 如果不是 Codex CLI 请求，则进行适配
    if (!isCodexCLI) {
      // 移除不需要的请求体字段
      cleanupCodexRequestBody(req.body)
    ...
    } else if (isLegacyCodexCLI) {
      cleanupCodexRequestBody(req.body)
      normalizeLegacyCodexInput(req.body)
      logger.info(
        `🧩 Legacy Codex CLI detected (${codexCliInfo?.client || 'codex'}/${codexCliInfo?.version || 'unknown'}), applying compatibility cleanup`
      )
    } else {
    ...
    // 构造 Codex 上游需要的受控请求头（兼容旧版 Codex CLI header 命名）
    const headers = buildCodexUpstreamHeaders(req.headers)
    if (!headers['session_id'] && sessionId) {
      headers['session_id'] = sessionId
    }
    if (!headers['originator'] && codexCliInfo?.client) {
      headers['originator'] = codexCliInfo.client
    }
    ...
    if (!isStream) {
      ...
      try {
        ...
        if (isLegacyCodexCLI && upstream.status !== 200) {
          const safeForwardHeaders = {
            version: headers['version'],
            'openai-beta': headers['openai-beta'],
            'openai-version': headers['openai-version'],
            originator: headers['originator'],
            session_id: headers['session_id']
          }

          const bodyKeys =
            req.body && typeof req.body === 'object' ? Object.keys(req.body).sort() : []
          ...
          if (responseData && typeof responseData === 'object' && !Buffer.isBuffer(responseData)) {
            if (
              ...
              upstreamMessage = responseData.error.message
            } else if (typeof responseData.message === 'string' && responseData.message) {
              upstreamMessage = responseData.message
            } else {
              errorSnippet = JSON.stringify(responseData).slice(0, 200)
            }
          } else if (typeof responseData === 'string' && responseData.trim()) {
            errorSnippet = responseData.trim().slice(0, 200)
          }

          logger.warn('🧩 Legacy Codex CLI upstream returned error', {
            status: upstream.status,
            userAgent,
            forwardHeaders: safeForwardHeaders,
            bodyKeys,
            upstreamMessage,
            errorSnippet: upstreamMessage ? null : errorSnippet
          })
        }
    ...
    upstream.data.on('end', async () => {
      ...
      if (isLegacyCodexCLI && upstream.status !== 200) {
        ...
        const errorSnippet = buffer.trim().slice(0, 4096)
        let upstreamMessage = null
        try {
        ...
        logger.warn('🧩 Legacy Codex CLI upstream returned error', {
          ...
          bodyKeys,
          upstreamMessage,
          errorSnippet: upstreamMessage ? null : errorSnippet.slice(0, 200)
        })
      }
